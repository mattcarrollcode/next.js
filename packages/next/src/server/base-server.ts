...
export default abstract class Server<ServerOptions extends Options = Options> {
  ...
  protected readonly handlers: ReadonlyArray<RouteHandler>
  ...
  public constructor(options: ServerOptions) {
    ...
    const handlers: Array<RouteHandler> = [
      ...
      this.handleNextDataRequest,
    ]

    if (this.minimalMode && this.hasAppDir) {
      handlers.push(this.handleRSCRequest)
    }
  ...
  protected handle: RouteHandler = async (req, res, url) => {
    let finished: boolean | void
    for (const handler of this.handlers) {
      finished = await handler(req, res, url)
